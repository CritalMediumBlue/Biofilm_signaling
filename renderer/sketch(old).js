...
function setup(){
	...
	Engine.velocityIterations=3; //the lower the faster it runs and the less quality the mechanical simulation has
...
function mouseClicked() {
  ...
  //	clearInterval(refresh);
	...
	for  (var y =0;  y < 1; y++){
		let new_bac=new Bacteria(Math.floor(mouseX / 40)*40+40/2, Math.floor(mouseY / 40)*40+40/2,30,random(-PI,PI),next_color, 2.5, 0.5, 1, 1);
...
for(var j=0;j<=50;j++){
	for (var i=0;i<=50;i++){
		...
		fill(255-n0[i][j]*(255/4),255,255-n0[i][j]*(255/2),255);
		rect(40*i+1,40*j+1,40,40);
...
if (show_nutrients==0){
	for(var j=0;j<=50;j++){
		for (var i=0;i<=50;i++){
			...
			fill(255,255-u0[i][j]*(255),255,255);
			rect(40*i+1,40*j+1,40,40);
...
//rect(N,1,58,N);
stroke(0);
/* for (var i =0;  i < walls_array.length; i++){
	walls_array[i].show_1();
 } */
